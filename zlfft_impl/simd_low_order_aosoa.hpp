...
namespace zlfft {
    ...
    public:
        explicit SIMDLowOrderAOSOA(const size_t order) :
            order_(order) {
            if (order < 4) {
                return;
            }
        ...
        void forward(std::span<C> in_buffer, std::span<C> out_buffer) {
            ...
            for (size_t i = 1; i < stages_.size() - 1; ++i) {
                ...
                case common::StageType::kRadix4Width4: {
                    common::radix4_width4_aosoa(in_aosoa, out_aosoa, n, w_r_ptr, w_i_ptr);
                    w_r_ptr += 12;
                    w_i_ptr += 12;
                    width = width << 2;
                    break;
                ...
                case common::StageType::kRadix4: {
                    common::radix4_aosoa(in_aosoa, out_aosoa, n, width, w_r_ptr, w_i_ptr);
                    w_r_ptr += 3 * width;
                    w_i_ptr += 3 * width;
                    width = width << 2;
                    break;
                ...
                case common::StageType::kRadix8: {
                    common::radix8_aosoa(in_aosoa, out_aosoa, n, width, w_r_ptr, w_i_ptr);
                    w_r_ptr += 7 * width; 
                    w_i_ptr += 7 * width;
                    width = width << 3;
                    break;
            ...
            common::radix4_last_pass_fused_aosoa(in_aosoa, out_buffer.data(), n, width, w_r_ptr, w_i_ptr);
        }

...
namespace zlfft {
    ...
    public:
        explicit SIMDLowOrder(const size_t order) :
            order_(order) {
            if (order < 4) {
                return;
            }
    ...
    private:
        ...
        static void radix8_first_pass_fused(const C* __restrict in,
            ...
            for (; j < m; ++j) {
                ...
                out_r[out_idx] = y00_r + v0_r;
                out_i[out_idx] = y00_i + v0_i;
        ...
        F* __restrict out_r, F* __restrict out_i,
            ...
            const F* __restrict const w2_r_base = w_r_ptr + 4;
            const F* __restrict const w2_i_base = w_i_ptr + 4;
            const F* __restrict const w3_r_base = w_r_ptr + 8;
            const F* __restrict const w3_i_base = w_i_ptr + 8;
            ...
            for (size_t j = 0; j < quarter_n; j += 4) {
                ...
                F* __restrict out_1_r = out_0_r + 4;
                F* __restrict out_1_i = out_0_i + 4;
                F* __restrict out_2_r = out_1_r + 4;
                F* __restrict out_2_i = out_1_i + 4;
                F* __restrict out_3_r = out_2_r + 4;
                F* __restrict out_3_i = out_2_i + 4;

                const auto i1 = hn::Load(d, in_1_i);
                const auto r1 = hn::Load(d, in_1_r);
                const auto t1_r = hn::NegMulAdd(i1, hn::Load(d, w1_i_base),
                                                hn::Mul(r1, hn::Load(d, w1_r_base)));
                const auto t1_i = hn::MulAdd(i1, hn::Load(d, w1_r_base),
                                             hn::Mul(r1, hn::Load(d, w1_i_base)));

                const auto i3 = hn::Load(d, in_3_i);
                const auto r3 = hn::Load(d, in_3_r);
                const auto t3_r = hn::NegMulAdd(i3, hn::Load(d, w3_i_base),
                                                hn::Mul(r3, hn::Load(d, w3_r_base)));
                const auto t3_i = hn::MulAdd(i3, hn::Load(d, w3_r_base),
                                             hn::Mul(r3, hn::Load(d, w3_i_base)));
                ...
                const auto i2 = hn::Load(d, in_2_i);
                const auto r2 = hn::Load(d, in_2_r);
                const auto t2_r = hn::NegMulAdd(i2, hn::Load(d, w2_i_base),
                                                hn::Mul(r2, hn::Load(d, w2_r_base)));
                const auto t2_i = hn::MulAdd(i2, hn::Load(d, w2_r_base),
                                             hn::Mul(r2, hn::Load(d, w2_i_base)));

                auto r0 = hn::Load(d, in_0_r);
                auto i0 = hn::Load(d, in_0_i);
                ...
                hn::Store(hn::Add(s0_r, s2_r), d, out_0_r);
                hn::Store(hn::Add(s0_i, s2_i), d, out_0_i);
                hn::Store(hn::Sub(s0_r, s2_r), d, out_2_r);
                hn::Store(hn::Sub(s0_i, s2_i), d, out_2_i);

                hn::Store(hn::Add(s1_r, s3_i), d, out_1_r);
                hn::Store(hn::Sub(s1_i, s3_r), d, out_1_i);
                hn::Store(hn::Sub(s1_r, s3_i), d, out_3_r);
                hn::Store(hn::Add(s1_i, s3_r), d, out_3_i);
            }
        }

...
namespace zlfft {
    ...
    public:
        explicit SIMDLowOrderOPT2(const size_t order) :
            ...
            if (order == 4) {
                stages_ = {kRadix4FirstPass, kRadix4LastPass};
            } else if (order == 5) {
                stages_ = {kRadix8FirstPass, kRadix4LastPass};
            } else if (order == 6) {
        ...
        void forward(std::span<C> in_buffer, std::span<C> out_buffer) {
            ...
            for (size_t i = 1; i < stages_.size() - 1; ++i) {
                ...
                switch (stage) {
                case kRadix4Width4: {
                    radix4_width4(in_r, in_i, out_r, out_i, n, w_r_ptr, w_i_ptr);
                    w_r_ptr += 12;
                    w_i_ptr += 12;
                    width = width << 2;
                    break;
    ...
    private:
        ...
        static void radix4_width4(const F* __restrict in_r, const F* __restrict in_i,
        ...
        static void radix4(const F* __restrict in_r, const F* __restrict in_i,
                           F* __restrict out_r, F* __restrict out_i,

...
namespace zlfft {
    ...
    template <typename F>
    class SIMDStockhamRadix4SOAKernel4OPT1 {
        using C = std::complex<F>;

    public:
        explicit SIMDStockhamRadix4SOAKernel4OPT1(const size_t order) :
            order_(order) {
            ...
            for (size_t j = 0; j < quarter_n; j += width) {
                ...
                if (use_simd) {
                    for (; k <= width - lanes; k += lanes) {
                        ...
                        hn::StoreInterleaved2(hn::Add(s0_r, s2_r), hn::Add(s0_i, s2_i), d, reinterpret_cast<F*>(out_0 + k));
                        hn::StoreInterleaved2(hn::Add(s1_r, s3_i), hn::Sub(s1_i, s3_r), d, reinterpret_cast<F*>(out_1 + k));
                        hn::StoreInterleaved2(hn::Sub(s0_r, s2_r), hn::Sub(s0_i, s2_i), d, reinterpret_cast<F*>(out_2 + k));
                        hn::StoreInterleaved2(hn::Sub(s1_r, s3_i), hn::Add(s1_i, s3_r), d, reinterpret_cast<F*>(out_3 + k));
                    }
                }

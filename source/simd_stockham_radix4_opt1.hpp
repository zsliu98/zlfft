...
namespace zlfft {
    ...
    template <typename F>
    class SIMDStockhamRadix4OPT1 {
        using C = std::complex<F>;

    public:
        explicit SIMDStockhamRadix4OPT1(const size_t order) :
            order_(order) {
        ...
        void forward(std::span<C> in_buffer, std::span<C> out_buffer) {
            ...
            if (width <= lanes) {
                if (width == 4) {
                    radix4_capped<4>(in, out, n, w_ptr);
                } else {
                    radix4_capped<8>(in, out, n, w_ptr);
                }
                w_ptr += 3 * width;
                std::swap(in, out);
                width <<= 2;
    ...
    private:
        ...
        void radix4_capped(const C* __restrict in, C* __restrict out, const size_t n, const C* __restrict w_ptr) {
            const size_t quarter_n = n >> 2;
            const size_t half_n = n >> 1;
            const size_t three_quarter_n = quarter_n * 3;

            const hn::CappedTag<F, width> d;
            const size_t lanes = hn::Lanes(d);
            ...
            for (size_t j = 0; j < quarter_n; j += width) {
                ...
                hn::LoadInterleaved2(d, reinterpret_cast<const F*>(in_0), r0, i0);
                hn::LoadInterleaved2(d, reinterpret_cast<const F*>(in_1), r1, i1);
                hn::LoadInterleaved2(d, reinterpret_cast<const F*>(in_2), r2, i2);
                hn::LoadInterleaved2(d, reinterpret_cast<const F*>(in_3), r3, i3);

                hn::LoadInterleaved2(d, w1_ptr, w1_r, w1_i);
                hn::LoadInterleaved2(d, w2_ptr, w2_r, w2_i);
                hn::LoadInterleaved2(d, w3_ptr, w3_r, w3_i);
                ...
                hn::StoreInterleaved2(hn::Add(s0_r, s2_r), hn::Add(s0_i, s2_i),
                                      d, reinterpret_cast<F*>(out_0));
                hn::StoreInterleaved2(hn::Add(s1_r, s3_i), hn::Sub(s1_i, s3_r),
                                      d, reinterpret_cast<F*>(out_1));
                hn::StoreInterleaved2(hn::Sub(s0_r, s2_r), hn::Sub(s0_i, s2_i),
                                      d, reinterpret_cast<F*>(out_2));
                hn::StoreInterleaved2(hn::Sub(s1_r, s3_i), hn::Add(s1_i, s3_r),
                                      d, reinterpret_cast<F*>(out_3));
            }
        }
